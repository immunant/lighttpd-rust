...
unsafe extern "C" fn mod_sockproxy_merge_config_cpv(
...
unsafe extern "C" fn mod_sockproxy_merge_config(
    pconf: *mut plugin_config,
...
unsafe extern "C" fn mod_sockproxy_patch_config(
    r: *mut request_st,
    p: *mut plugin_data,
) {
    memcpy(
...
#[cold]
unsafe extern "C" fn mod_sockproxy_set_defaults(
    mut srv: *mut server,
        ...
        while -(1 as libc::c_int) != (*cpv).k_id {
            match (*cpv).k_id {
                0 => {
                    ...
                    if gw.is_null() {
                        ck_assert_failed(
                            b"src/mod_sockproxy.c\0" as *const u8 as *const libc::c_char,
        ...
        && (*(*p).cvlist).v.u2[1 as libc::c_int as usize] != 0
    {
        ...
        if -(1 as libc::c_int) != (*cpv_0).k_id {
            mod_sockproxy_merge_config(&mut (*p).defaults, cpv_0);
        }
    }
    return HANDLER_GO_ON;
}
unsafe extern "C" fn sockproxy_create_env_connect(
    ...
    status_counter_inc(
        b"sockproxy.requests\0" as *const u8 as *const libc::c_char,
        (::std::mem::size_of::<[libc::c_char; 19]>() as libc::c_ulong as uint32_t)
            .wrapping_sub(1 as libc::c_int as libc::c_uint) as size_t,
    );
    return HANDLER_GO_ON;
}
unsafe extern "C" fn mod_sockproxy_connection_accept(
    ...
    rc = gw_check_extension(r, p, 1 as libc::c_int, 0 as libc::c_int as size_t);
    ...
    if (*r).handler_module == (*p).self_0 as *const plugin {
        ...
        (*hctx)
            .create_env = Some(
            sockproxy_create_env_connect
                as unsafe extern "C" fn(*mut handler_ctx) -> handler_t,
        );
        (*hctx).response = chunk_buffer_acquire();
        (*r).http_status = -(1 as libc::c_int);
    }
    return HANDLER_GO_ON;
}
#[no_mangle]
pub unsafe extern "C" fn mod_sockproxy_plugin_init(mut p: *mut plugin) -> libc::c_int {
    (*p).version = 0x10440 as libc::c_int as size_t;
    (*p).name = b"sockproxy\0" as *const u8 as *const libc::c_char;
    (*p)
    ...
    (*p)
        ...
        mod_sockproxy_connection_accept
    ...
    cpk = [
        {
            let mut init = config_plugin_keys_t {
                k: b"sockproxy.server\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"sockproxy.balance\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"sockproxy.debug\0" as *const u8 as *const libc::c_char,
