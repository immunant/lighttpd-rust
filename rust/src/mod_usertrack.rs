...
extern "C" {
    ...
    fn buffer_append_string_encoded_hex_lc(
        b: *mut buffer,
        s: *const libc::c_char,
        len: size_t,
    );
...
pub struct plugin_config {
    pub cookie_name: *const buffer,
    pub cookie_attrs: *const buffer,
    pub cookie_domain: *const buffer,
    pub cookie_max_age: libc::c_uint,
}
#[derive(Copy, Clone)]
#[repr(C)]
...
#[cold]
unsafe extern "C" fn mod_usertrack_init() -> *mut libc::c_void {
    return calloc(
        1 as libc::c_int as libc::c_ulong,
        ::std::mem::size_of::<plugin_data>() as libc::c_ulong,
    );
}
unsafe extern "C" fn mod_usertrack_merge_config_cpv(
...
unsafe extern "C" fn mod_usertrack_merge_config(
    pconf: *mut plugin_config,
...
unsafe extern "C" fn mod_usertrack_patch_config(
    r: *mut request_st,
    p: *mut plugin_data,
) {
    (*p).conf = (*p).defaults;
...
#[cold]
unsafe extern "C" fn mod_usertrack_set_defaults(
    mut srv: *mut server,
    ...
    if config_plugin_values_init(
        ...
        cpk.as_ptr(),
        b"mod_usertrack\0" as *const u8 as *const libc::c_char,
    ) == 0
    {
            ...
            match (*cpv).k_id {
                0 => {
                    if buffer_is_blank((*cpv).v.b) == 0 {
                        ...
                        let len: size_t = buffer_clen((*cpv).v.b) as size_t;
                        let mut j: size_t = 0 as libc::c_int as size_t;
                        while j < len {
                            if light_isalpha(*ptr.offset(j as isize) as libc::c_int) == 0
                            {
                                log_error(
                                    (*srv).errh,
                                    b"src/mod_usertrack.c\0" as *const u8
                                        as *const libc::c_char,
                                    102 as libc::c_int as libc::c_uint,
                                    b"invalid character in %s: %s\0" as *const u8
                                        as *const libc::c_char,
                ...
                2 => {
                    if buffer_is_blank((*cpv).v.b) == 0 {
                        let ptr_0: *const libc::c_char = (*(*cpv).v.b).ptr;
                        let len_0: size_t = buffer_clen((*cpv).v.b) as size_t;
                        let mut j_0: size_t = 0 as libc::c_int as size_t;
                        while j_0 < len_0 {
                            let c: libc::c_char = *ptr_0.offset(j_0 as isize);
        ...
        && (*(*p).cvlist).v.u2[1 as libc::c_int as usize] != 0
    {
        ...
        if -(1 as libc::c_int) != (*cpv_0).k_id {
            mod_usertrack_merge_config(&mut (*p).defaults, cpv_0);
        }
    }
    if ((*p).defaults.cookie_name).is_null() {
        static mut default_cookie_name: C2RustUnnamed_6 = {
            let mut init = C2RustUnnamed_6 {
                ptr: b"TRACKID\0" as *const u8 as *const libc::c_char,
        ...
        let ref mut fresh0 = *(&mut (*p).defaults.cookie_name as *mut *const buffer);
        *fresh0 = &default_cookie_name as *const C2RustUnnamed_6 as *const buffer;
    }
    return HANDLER_GO_ON;
}
#[inline(never)]
unsafe extern "C" fn mod_usertrack_set_cookie(
    r: *mut request_st,
    p: *mut plugin_data,
) -> handler_t {
    let mut rnd: libc::c_int = li_rand_pseudo();
    let mut iov: [const_iovec; 4] = [
        {
            let mut init = const_iovec {
                iov_base: (*r).uri.path.ptr as *const libc::c_void,
                iov_len: buffer_clen(&mut (*r).uri.path) as size_t,
            };
            init
        },
        {
        ...
        {
            let mut init = const_iovec {
                iov_base: &mut log_epoch_secs as *mut unix_time64_t
                    as *const libc::c_void,
    ...
    let mut h: [libc::c_uchar; 16] = [0; 16];
    MD5_iov(
        h.as_mut_ptr(),
        iov.as_mut_ptr(),
    ...
    let cookie: *mut buffer = (*r).tmp_buf;
    buffer_clear(cookie);
    buffer_append_str2(
        cookie,
        (*(*p).conf.cookie_name).ptr,
        buffer_clen((*p).conf.cookie_name) as size_t,
    ...
    buffer_append_string_encoded_hex_lc(
        cookie,
        h.as_mut_ptr() as *mut libc::c_char,
        ::std::mem::size_of::<[libc::c_uchar; 16]>() as libc::c_ulong,
    );
    if !((*p).conf.cookie_attrs).is_null() {
        buffer_append_buffer(cookie, (*p).conf.cookie_attrs);
    ...
    buffer_append_string_len(
        cookie,
        b"; Path=/; Version=1\0" as *const u8 as *const libc::c_char,
        (::std::mem::size_of::<[libc::c_char; 20]>() as libc::c_ulong as uint32_t)
            .wrapping_sub(1 as libc::c_int as libc::c_uint) as size_t,
    );
    if !((*p).conf.cookie_domain).is_null() {
        buffer_append_string_len(
            cookie,
        ...
        buffer_append_string_encoded(
            cookie,
            (*(*p).conf.cookie_domain).ptr,
            buffer_clen((*p).conf.cookie_domain) as size_t,
            ENCODING_REL_URI,
        );
    }
    if (*p).conf.cookie_max_age != 0 {
        buffer_append_string_len(
            cookie,
        ...
        buffer_append_int(cookie, (*p).conf.cookie_max_age as intmax_t);
    }
    http_header_response_insert(
        r,
        HTTP_HEADER_SET_COOKIE,
...
unsafe extern "C" fn mod_usertrack_uri_handler(
    mut r: *mut request_st,
    mut p_d: *mut libc::c_void,
) -> handler_t {
    let p: *mut plugin_data = p_d as *mut plugin_data;
    mod_usertrack_patch_config(r, p);
    if ((*p).conf.cookie_name).is_null() {
        return HANDLER_GO_ON;
    }
    let b: *const buffer = http_header_request_get(
        r,
        HTTP_HEADER_COOKIE,
    ...
    if !b.is_null() {
        let g: *const libc::c_char = strstr((*b).ptr, (*(*p).conf.cookie_name).ptr);
        if !g.is_null() {
            let mut nc: *const libc::c_char = g
                .offset(buffer_clen((*p).conf.cookie_name) as isize);
            while *nc as libc::c_int == ' ' as i32 || *nc as libc::c_int == '\t' as i32 {
                nc = nc.offset(1);
            }
            if *nc as libc::c_int == '=' as i32 {
                if strlen(nc) > 32 as libc::c_int as libc::c_ulong {
                    return HANDLER_GO_ON;
                }
            }
        }
...
#[no_mangle]
pub unsafe extern "C" fn mod_usertrack_plugin_init(mut p: *mut plugin) -> libc::c_int {
    (*p).version = 0x10440 as libc::c_int as size_t;
    (*p).name = b"usertrack\0" as *const u8 as *const libc::c_char;
    (*p)
        .init = ::std::mem::transmute::<
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
    >(Some(mod_usertrack_init as unsafe extern "C" fn() -> *mut libc::c_void));
    (*p)
    ...
    cpk = [
        {
            let mut init = config_plugin_keys_t {
                k: b"usertrack.cookie-name\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"usertrack.cookie-max-age\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"usertrack.cookie-domain\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"usertrack.cookie-attrs\0" as *const u8 as *const libc::c_char,
