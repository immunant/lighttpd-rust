...
pub struct plugin_config {
    pub max_conns: libc::c_ushort,
    pub silent: libc::c_ushort,
    pub location: *const buffer,
}
#[derive(Copy, Clone)]
#[repr(C)]
...
#[cold]
unsafe extern "C" fn mod_evasive_init() -> *mut libc::c_void {
    return calloc(
        1 as libc::c_int as libc::c_ulong,
        ::std::mem::size_of::<plugin_data>() as libc::c_ulong,
    );
}
unsafe extern "C" fn mod_evasive_merge_config_cpv(
    ...
    match (*cpv).k_id {
        ...
        1 => {
            (*pconf)
                .silent = (0 as libc::c_int as libc::c_uint != (*cpv).v.u) as libc::c_int
                as libc::c_ushort;
        }
        2 => {
...
unsafe extern "C" fn mod_evasive_merge_config(
    pconf: *mut plugin_config,
...
unsafe extern "C" fn mod_evasive_patch_config(r: *mut request_st, p: *mut plugin_data) {
    (*p).conf = (*p).defaults;
...
#[cold]
unsafe extern "C" fn mod_evasive_set_defaults(
    mut srv: *mut server,
    ...
    if config_plugin_values_init(
        ...
        cpk.as_ptr(),
        b"mod_evasive\0" as *const u8 as *const libc::c_char,
    ) == 0
    {
        ...
        && (*(*p).cvlist).v.u2[1 as libc::c_int as usize] != 0
    {
        ...
        if -(1 as libc::c_int) != (*cpv_0).k_id {
            mod_evasive_merge_config(&mut (*p).defaults, cpv_0);
        }
    }
...
#[cold]
#[inline(never)]
unsafe extern "C" fn mod_evasive_reached_per_ip_limit(
    r: *mut request_st,
    p: *const plugin_data,
) -> handler_t {
    if (*p).conf.silent == 0 {
        log_error(
            (*r).conf.errh,
            b"src/mod_evasive.c\0" as *const u8 as *const libc::c_char,
            142 as libc::c_int as libc::c_uint,
            b"%s turned away. Too many connections.\0" as *const u8
                as *const libc::c_char,
            (*(*r).con).dst_addr_buf.ptr,
        );
    }
    if !((*p).conf.location).is_null() {
...
unsafe extern "C" fn mod_evasive_check_per_ip_limit(
    r: *mut request_st,
    p: *const plugin_data,
    mut c: *const connection,
) -> handler_t {
    let dst_addr: *const sock_addr = &mut (*(*r).con).dst_addr;
    let mut conns_by_ip: uint_fast32_t = 0 as libc::c_int as uint_fast32_t;
    while !c.is_null() {
        if (*c).request.state as libc::c_uint
            > CON_STATE_REQUEST_END as libc::c_int as libc::c_uint
            && sock_addr_is_addr_eq(&(*c).dst_addr, dst_addr) != 0
            && {
                conns_by_ip = conns_by_ip.wrapping_add(1);
                conns_by_ip > (*p).conf.max_conns as libc::c_ulong
            }
        {
            return mod_evasive_reached_per_ip_limit(r, p);
        }
        c = (*c).next;
    }
    return HANDLER_GO_ON;
}
unsafe extern "C" fn mod_evasive_uri_handler(
    mut r: *mut request_st,
    mut p_d: *mut libc::c_void,
) -> handler_t {
    let p: *mut plugin_data = p_d as *mut plugin_data;
    mod_evasive_patch_config(r, p);
    return (if (*p).conf.max_conns as libc::c_int == 0 as libc::c_int {
        HANDLER_GO_ON as libc::c_int as libc::c_uint
    } else {
        mod_evasive_check_per_ip_limit(r, p, (*(*(*r).con).srv).conns) as libc::c_uint
    }) as handler_t;
}
#[no_mangle]
pub unsafe extern "C" fn mod_evasive_plugin_init(mut p: *mut plugin) -> libc::c_int {
    (*p).version = 0x10440 as libc::c_int as size_t;
    (*p).name = b"evasive\0" as *const u8 as *const libc::c_char;
    (*p)
        .init = ::std::mem::transmute::<
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
    >(Some(mod_evasive_init as unsafe extern "C" fn() -> *mut libc::c_void));
    (*p)
        .set_defaults = Some(
    ...
    cpk = [
        {
            let mut init = config_plugin_keys_t {
                k: b"evasive.max-conns-per-ip\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"evasive.silent\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"evasive.location\0" as *const u8 as *const libc::c_char,
