...
#[cold]
unsafe extern "C" fn mod_indexfile_init() -> *mut libc::c_void {
    return calloc(
        1 as libc::c_int as libc::c_ulong,
        ::std::mem::size_of::<plugin_data>() as libc::c_ulong,
    );
}
unsafe extern "C" fn mod_indexfile_merge_config_cpv(
...
unsafe extern "C" fn mod_indexfile_merge_config(
    pconf: *mut plugin_config,
...
unsafe extern "C" fn mod_indexfile_patch_config(
    r: *mut request_st,
    p: *mut plugin_data,
) {
    (*p).conf = (*p).defaults;
...
#[cold]
unsafe extern "C" fn mod_indexfile_set_defaults(
    mut srv: *mut server,
        ...
        && (*(*p).cvlist).v.u2[1 as libc::c_int as usize] != 0
    {
        ...
        if -(1 as libc::c_int) != (*cpv).k_id {
            mod_indexfile_merge_config(&mut (*p).defaults, cpv);
        }
    }
    return HANDLER_GO_ON;
}
unsafe extern "C" fn mod_indexfile_tryfiles(
    r: *mut request_st,
    indexfiles: *const array,
) -> handler_t {
    let mut k: uint32_t = 0 as libc::c_int as uint32_t;
    while k < (*indexfiles).used {
        let v: *const buffer = &mut (*(*((*indexfiles).data).offset(k as isize)
            as *mut data_string))
            .value;
        let b: *mut buffer = if *((*v).ptr).offset(0 as libc::c_int as isize)
        ...
        buffer_append_path_len(b, (*v).ptr, buffer_clen(v) as size_t);
        let st: *const stat_cache_st = stat_cache_path_stat(b);
        ...
        if st.is_null() {
            match *__errno_location() {
                ...
                _ => {
                    ...
                    log_perror(
                        (*r).conf.errh,
                        b"src/mod_indexfile.c\0" as *const u8 as *const libc::c_char,
                        110 as libc::c_int as libc::c_uint,
                        b"index file error for request: %s -> %s\0" as *const u8
            ...
            if *((*v).ptr).offset(0 as libc::c_int as isize) as libc::c_int == '/' as i32
            {
                buffer_copy_buffer(&mut (*r).uri.path, v);
                http_header_env_set(
                    r,
                    b"PATH_TRANSLATED_DIRINDEX\0" as *const u8 as *const libc::c_char,
                ...
                buffer_copy_path_len2(
                    ...
                    (*v).ptr,
                    buffer_clen(v) as size_t,
                );
            } else {
                buffer_append_buffer(&mut (*r).uri.path, v);
...
unsafe extern "C" fn mod_indexfile_subrequest(
    ...
    if (*r).conf.log_request_handling != 0 {
        log_error(
            ...
            b"-- handling the request as Indexfile\0" as *const u8 as *const libc::c_char,
        );
        log_error(
            (*r).conf.errh,
            b"src/mod_indexfile.c\0" as *const u8 as *const libc::c_char,
...
#[no_mangle]
pub unsafe extern "C" fn mod_indexfile_plugin_init(mut p: *mut plugin) -> libc::c_int {
    (*p).version = 0x10440 as libc::c_int as size_t;
    (*p).name = b"indexfile\0" as *const u8 as *const libc::c_char;
    (*p)
        .init = ::std::mem::transmute::<
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
    >(Some(mod_indexfile_init as unsafe extern "C" fn() -> *mut libc::c_void));
    (*p)
    ...
    cpk = [
        {
            let mut init = config_plugin_keys_t {
                k: b"index-file.names\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"server.indexfiles\0" as *const u8 as *const libc::c_char,
