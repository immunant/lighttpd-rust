...
extern "C" {
    ...
    fn array_match_value_suffix_nc(a: *const array, b: *const buffer) -> *const buffer;
    fn log_error(
...
pub struct plugin_config {
    pub access_allow: *const array,
    pub access_deny: *const array,
}
#[derive(Copy, Clone)]
#[repr(C)]
...
#[cold]
unsafe extern "C" fn mod_access_init() -> *mut libc::c_void {
    return calloc(
        1 as libc::c_int as libc::c_ulong,
        ::std::mem::size_of::<plugin_data>() as libc::c_ulong,
    );
}
unsafe extern "C" fn mod_access_merge_config_cpv(
...
unsafe extern "C" fn mod_access_merge_config(
    pconf: *mut plugin_config,
...
unsafe extern "C" fn mod_access_patch_config(r: *mut request_st, p: *mut plugin_data) {
    (*p).conf = (*p).defaults;
...
#[cold]
unsafe extern "C" fn mod_access_set_defaults(
    mut srv: *mut server,
    ...
    if config_plugin_values_init(
        ...
        cpk.as_ptr(),
        b"mod_access\0" as *const u8 as *const libc::c_char,
    ) == 0
    {
        ...
        && (*(*p).cvlist).v.u2[1 as libc::c_int as usize] != 0
    {
        ...
        if -(1 as libc::c_int) != (*cpv).k_id {
            mod_access_merge_config(&mut (*p).defaults, cpv);
        }
    }
    return HANDLER_GO_ON;
}
#[cold]
unsafe extern "C" fn mod_access_reject(
    r: *mut request_st,
    p: *mut plugin_data,
) -> handler_t {
    if (*r).conf.log_request_handling != 0 {
        if !((*p).conf.access_allow).is_null() && (*(*p).conf.access_allow).used != 0 {
            log_error(
                (*r).conf.errh,
                b"src/mod_access.c\0" as *const u8 as *const libc::c_char,
                86 as libc::c_int as libc::c_uint,
                b"url denied as failed to match any from access_allow %s\0" as *const u8
            ...
            log_error(
                ...
                b"url denied as we match access_deny %s\0" as *const u8
                    as *const libc::c_char,
                (*r).uri.path.ptr,
            );
        }
...
unsafe extern "C" fn mod_access_check(
    allow: *const array,
    deny: *const array,
    urlpath: *const buffer,
    lc: libc::c_int,
) -> libc::c_int {
    if !allow.is_null() && (*allow).used != 0 {
        let match_0: *const buffer = if lc == 0 {
            array_match_value_suffix(allow, urlpath)
        } else {
            array_match_value_suffix_nc(allow, urlpath)
        };
        return (match_0 != 0 as *mut libc::c_void as *const buffer) as libc::c_int;
    }
    if !deny.is_null() && (*deny).used != 0 {
        let match_1: *const buffer = if lc == 0 {
            array_match_value_suffix(deny, urlpath)
        } else {
            array_match_value_suffix_nc(deny, urlpath)
        };
        return (match_1 == 0 as *mut libc::c_void as *const buffer) as libc::c_int;
    }
    return 1 as libc::c_int;
}
unsafe extern "C" fn mod_access_uri_handler(
    mut r: *mut request_st,
    mut p_d: *mut libc::c_void,
) -> handler_t {
    let mut p: *mut plugin_data = p_d as *mut plugin_data;
    mod_access_patch_config(r, p);
    if ((*p).conf.access_allow).is_null() && ((*p).conf.access_deny).is_null() {
        return HANDLER_GO_ON;
    }
    return (if mod_access_check(
        (*p).conf.access_allow,
        (*p).conf.access_deny,
        &mut (*r).uri.path,
        (*r).conf.force_lowercase_filenames as libc::c_int,
    ) != 0
    {
        HANDLER_GO_ON as libc::c_int as libc::c_uint
    } else {
        mod_access_reject(r, p) as libc::c_uint
    }) as handler_t;
}
#[no_mangle]
pub unsafe extern "C" fn mod_access_plugin_init(mut p: *mut plugin) -> libc::c_int {
    (*p).version = 0x10440 as libc::c_int as size_t;
    (*p).name = b"access\0" as *const u8 as *const libc::c_char;
    (*p)
        .init = ::std::mem::transmute::<
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
    >(Some(mod_access_init as unsafe extern "C" fn() -> *mut libc::c_void));
    (*p)
        .set_defaults = Some(
    ...
    cpk = [
        {
            let mut init = config_plugin_keys_t {
                k: b"url.access-deny\0" as *const u8 as *const libc::c_char,
        ...
        {
            let mut init = config_plugin_keys_t {
                k: b"url.access-allow\0" as *const u8 as *const libc::c_char,
