...
extern "C" {
    ...
    fn array_match_key_prefix_klen(
        a: *const array,
        s: *const libc::c_char,
        slen: uint32_t,
    ) -> *mut data_unset;
    fn array_match_key_prefix_nc_klen(
        a: *const array,
        s: *const libc::c_char,
        slen: uint32_t,
    ) -> *mut data_unset;
...
#[cold]
unsafe extern "C" fn mod_alias_init() -> *mut libc::c_void {
    return calloc(
        1 as libc::c_int as libc::c_ulong,
        ::std::mem::size_of::<plugin_data>() as libc::c_ulong,
    );
}
unsafe extern "C" fn mod_alias_merge_config_cpv(
...
unsafe extern "C" fn mod_alias_merge_config(
    pconf: *mut plugin_config,
...
unsafe extern "C" fn mod_alias_patch_config(r: *mut request_st, p: *mut plugin_data) {
    (*p).conf = (*p).defaults;
...
unsafe extern "C" fn mod_alias_check_order(
    srv: *mut server,
    a: *const array,
) -> libc::c_int {
    let mut j: uint32_t = 0 as libc::c_int as uint32_t;
    while j < (*a).used {
        let prefix: *const buffer = &mut (**((*a).data).offset(j as isize)).key;
        let plen: size_t = buffer_clen(prefix) as size_t;
        let mut k: uint32_t = j.wrapping_add(1 as libc::c_int as libc::c_uint);
        while k < (*a).used {
            let key: *const buffer = &mut (**((*a).data).offset(k as isize)).key;
            if (buffer_clen(key) as libc::c_ulong) < plen {
                break;
            }
            if memcmp(
                (*key).ptr as *const libc::c_void,
                (*prefix).ptr as *const libc::c_void,
                plen,
            ) != 0 as libc::c_int
            {
                break;
            }
            let mut dj: *const data_unset = *((*a).data).offset(j as isize);
            let mut dk: *const data_unset = *((*a).data).offset(k as isize);
            let mut data: *mut *const data_unset = (*a).data as *mut *const data_unset;
            while *data != dj && *data != dk {
                data = data.offset(1);
            }
            if *data == dj {
                log_error(
                    (*srv).errh,
                    b"src/mod_alias.c\0" as *const u8 as *const libc::c_char,
                    70 as libc::c_int as libc::c_uint,
                    b"url.alias: `%s' will never match as `%s' matched first\0"
...
#[cold]
unsafe extern "C" fn mod_alias_set_defaults(
    mut srv: *mut server,
    ...
    if config_plugin_values_init(
        ...
        cpk.as_ptr(),
        b"mod_alias\0" as *const u8 as *const libc::c_char,
    ) == 0
    {
    ...
    while i < (*p).nconfig {
        ...
        while -(1 as libc::c_int) != (*cpv).k_id {
            match (*cpv).k_id {
                0 => {
                    if (*(*cpv).v.a).used >= 2 as libc::c_int as libc::c_uint
                        && mod_alias_check_order(srv, (*cpv).v.a) == 0
                    {
                        return HANDLER_ERROR;
                    }
                }
        ...
        && (*(*p).cvlist).v.u2[1 as libc::c_int as usize] != 0
    {
        ...
        if -(1 as libc::c_int) != (*cpv_0).k_id {
            mod_alias_merge_config(&mut (*p).defaults, cpv_0);
        }
    }
    return HANDLER_GO_ON;
}
unsafe extern "C" fn mod_alias_remap(
    r: *mut request_st,
    aliases: *const array,
) -> handler_t {
    let mut basedir_len: uint32_t = buffer_clen(&mut (*r).physical.basedir);
    if buffer_has_pathsep_suffix(&mut (*r).physical.basedir) != 0 {
        basedir_len = basedir_len.wrapping_sub(1);
    }
    let path_len: uint32_t = buffer_clen(&mut (*r).physical.path);
    if 0 as libc::c_int as libc::c_uint == path_len || path_len < basedir_len {
        return HANDLER_GO_ON;
    }
    let uri_len: uint32_t = path_len.wrapping_sub(basedir_len);
    let mut uri_ptr: *const libc::c_char = ((*r).physical.path.ptr)
        .offset(basedir_len as isize);
    let ds: *mut data_string = (if (*r).conf.force_lowercase_filenames == 0 {
    ...
    let alias_len: uint32_t = buffer_clen(&mut (*ds).key);
    let vlen: uint32_t = buffer_clen(&mut (*ds).value);
    if *uri_ptr.offset(alias_len as isize) as libc::c_int == '.' as i32 {
        let mut s: *const libc::c_char = uri_ptr
            .offset(alias_len as isize)
            .offset(1 as libc::c_int as isize);
        if *s as libc::c_int == '.' as i32 {
            s = s.offset(1);
        }
        if *s as libc::c_int == '/' as i32 || *s as libc::c_int == '\u{0}' as i32 {
            if 0 as libc::c_int as libc::c_uint != alias_len
                && *((*ds).key.ptr)
                    .offset(
                        alias_len.wrapping_sub(1 as libc::c_int as libc::c_uint) as isize,
                    ) as libc::c_int != '/' as i32
                && 0 as libc::c_int as libc::c_uint != vlen
                && *((*ds).value.ptr)
                    .offset(vlen.wrapping_sub(1 as libc::c_int as libc::c_uint) as isize)
                    as libc::c_int == '/' as i32
            {
    ...
    if vlen != basedir_len.wrapping_add(alias_len) {
        let nlen: uint32_t = vlen.wrapping_add(uri_len).wrapping_sub(alias_len);
        if path_len.wrapping_add(buffer_string_space(&mut (*r).physical.path)) < nlen {
            buffer_string_prepare_append(
                &mut (*r).physical.path,
                nlen.wrapping_sub(path_len) as size_t,
            );
            uri_ptr = ((*r).physical.path.ptr).offset(basedir_len as isize);
        }
        memmove(
            ((*r).physical.path.ptr).offset(vlen as isize) as *mut libc::c_void,
            uri_ptr.offset(alias_len as isize) as *const libc::c_void,
            uri_len.wrapping_sub(alias_len) as libc::c_ulong,
        );
        buffer_truncate(&mut (*r).physical.path, nlen);
    }
    memcpy(
        (*r).physical.path.ptr as *mut libc::c_void,
        (*ds).value.ptr as *const libc::c_void,
        vlen as libc::c_ulong,
    );
    buffer_copy_string_len(&mut (*r).physical.basedir, (*ds).value.ptr, vlen as size_t);
    return HANDLER_GO_ON;
}
unsafe extern "C" fn mod_alias_physical_handler(
    mut r: *mut request_st,
    mut p_d: *mut libc::c_void,
) -> handler_t {
    let p: *mut plugin_data = p_d as *mut plugin_data;
    mod_alias_patch_config(r, p);
    return (if !((*p).conf.alias).is_null() {
        mod_alias_remap(r, (*p).conf.alias) as libc::c_uint
    } else {
        HANDLER_GO_ON as libc::c_int as libc::c_uint
    }) as handler_t;
}
#[no_mangle]
pub unsafe extern "C" fn mod_alias_plugin_init(mut p: *mut plugin) -> libc::c_int {
    (*p).version = 0x10440 as libc::c_int as size_t;
    (*p).name = b"alias\0" as *const u8 as *const libc::c_char;
    (*p)
        .init = ::std::mem::transmute::<
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
    >(Some(mod_alias_init as unsafe extern "C" fn() -> *mut libc::c_void));
    (*p)
    ...
    cpk = [
        {
            let mut init = config_plugin_keys_t {
                k: b"alias.url\0" as *const u8 as *const libc::c_char,
