...
#[cold]
unsafe extern "C" fn mod_evhost_init() -> *mut libc::c_void {
    return calloc(
        1 as libc::c_int as libc::c_ulong,
        ::std::mem::size_of::<plugin_data>() as libc::c_ulong,
    );
}
unsafe extern "C" fn mod_evhost_free_path_pieces(mut path_pieces: *const buffer) {
    let mut b: *mut buffer = 0 as *mut buffer;
    let ref mut fresh0 = *(&mut b as *mut *mut buffer as *mut *const buffer);
    *fresh0 = path_pieces;
    while !((*path_pieces).ptr).is_null() {
        free((*path_pieces).ptr as *mut libc::c_void);
        path_pieces = path_pieces.offset(1);
    }
    free(b as *mut libc::c_void);
}
#[cold]
unsafe extern "C" fn mod_evhost_free(mut p_d: *mut libc::c_void) {
    let p: *mut plugin_data = p_d as *mut plugin_data;
    array_free_data(&mut (*p).split_vals);
    if ((*p).cvlist).is_null() {
        return;
    }
    ...
    while i < used {
        ...
        while -(1 as libc::c_int) != (*cpv).k_id {
                ...
                || ((*cpv).v.v).is_null())
            {
                match (*cpv).k_id {
                    0 => {
                        mod_evhost_free_path_pieces((*cpv).v.v as *const buffer);
                    }
                    _ => {}
                }
            }
...
#[cold]
unsafe extern "C" fn mod_evhost_parse_pattern_err(mut bptr: *mut buffer) -> *mut buffer {
    while !((*bptr).ptr).is_null() {
        free((*bptr).ptr as *mut libc::c_void);
        bptr = bptr.offset(1);
    }
    return 0 as *mut buffer;
}
unsafe extern "C" fn mod_evhost_parse_pattern(
    ...
    let mut bptr: [buffer; 128] = [buffer {
        ptr: 0 as *mut libc::c_char,
        used: 0,
        size: 0,
    }; 128];
    memset(
        bptr.as_mut_ptr() as *mut libc::c_void,
        0 as libc::c_int,
        ::std::mem::size_of::<[buffer; 128]>() as libc::c_ulong,
    );
    pos = ptr;
    while *ptr != 0 {
        if *ptr as libc::c_int == '%' as i32 {
            let mut len: size_t = 0;
            if used >= sz.wrapping_sub(1 as libc::c_int as libc::c_uint) {
                return mod_evhost_parse_pattern_err(bptr.as_mut_ptr());
            }
            if *ptr.offset(1 as libc::c_int as isize) as libc::c_int == '%' as i32
                || *ptr.offset(1 as libc::c_int as isize) as libc::c_int == '_' as i32
                || light_isdigit(*ptr.offset(1 as libc::c_int as isize) as libc::c_int)
                    != 0
            {
                len = 2 as libc::c_int as size_t;
            } else if *ptr.offset(1 as libc::c_int as isize) as libc::c_int == '{' as i32
                {
                if light_isdigit(*ptr.offset(2 as libc::c_int as isize) as libc::c_int)
                    == 0
                {
                    return mod_evhost_parse_pattern_err(bptr.as_mut_ptr());
                }
                if *ptr.offset(3 as libc::c_int as isize) as libc::c_int == '.' as i32 {
                    if light_isdigit(
            ...
            buffer_copy_string_len(
                bptr.as_mut_ptr().offset(used as isize),
                pos,
                ptr.offset_from(pos) as libc::c_long as size_t,
            );
            pos = ptr.offset(len as isize);
            buffer_copy_string_len(
                bptr
                    .as_mut_ptr()
            ...
            used = (used as libc::c_uint).wrapping_add(2 as libc::c_int as libc::c_uint)
    ...
    let path_pieces: *mut buffer = malloc(
        (::std::mem::size_of::<[buffer; 128]>() as libc::c_ulong)
            .wrapping_add(
                (used.wrapping_add(1 as libc::c_int as libc::c_uint) as libc::c_ulong)
                    .wrapping_mul(::std::mem::size_of::<buffer>() as libc::c_ulong),
            ),
    ) as *mut buffer;
    if path_pieces.is_null() {
        ck_assert_failed(
            b"src/mod_evhost.c\0" as *const u8 as *const libc::c_char,
            130 as libc::c_int as libc::c_uint,
            b"path_pieces\0" as *const u8 as *const libc::c_char,
        );
    }
    return memcpy(
        path_pieces as *mut libc::c_void,
        bptr.as_mut_ptr() as *const libc::c_void,
        (used.wrapping_add(1 as libc::c_int as libc::c_uint) as libc::c_ulong)
            .wrapping_mul(::std::mem::size_of::<buffer>() as libc::c_ulong),
    ) as *mut buffer;
}
unsafe extern "C" fn mod_evhost_merge_config_cpv(
    ...
    match (*cpv).k_id {
        0 => {
            if (*cpv).vtype as libc::c_uint
                == T_CONFIG_LOCAL as libc::c_int as libc::c_uint
            {
                (*pconf).path_pieces = (*cpv).v.v as *const buffer;
            }
        }
        _ => return,
    };
}
unsafe extern "C" fn mod_evhost_merge_config(
    pconf: *mut plugin_config,
...
unsafe extern "C" fn mod_evhost_patch_config(r: *mut request_st, p: *mut plugin_data) {
    (*p).conf = (*p).defaults;
...
#[cold]
unsafe extern "C" fn mod_evhost_set_defaults(
    mut srv: *mut server,
    ...
    if config_plugin_values_init(
        ...
        cpk.as_ptr(),
        b"mod_evhost\0" as *const u8 as *const libc::c_char,
    ) == 0
    {
            ...
            match (*cpv).k_id {
                0 => {
                    if buffer_is_blank((*cpv).v.b) == 0 {
                        let ptr: *const libc::c_char = (*(*cpv).v.b).ptr;
                        (*cpv).v.v = mod_evhost_parse_pattern(ptr) as *mut libc::c_void;
                        if ((*cpv).v.v).is_null() {
                            log_error(
                                (*srv).errh,
                                b"src/mod_evhost.c\0" as *const u8 as *const libc::c_char,
                                185 as libc::c_int as libc::c_uint,
                                b"invalid evhost.path-pattern: %s\0" as *const u8
        ...
        && (*(*p).cvlist).v.u2[1 as libc::c_int as usize] != 0
    {
        ...
        if -(1 as libc::c_int) != (*cpv_0).k_id {
            mod_evhost_merge_config(&mut (*p).defaults, cpv_0);
        }
    }
    return HANDLER_GO_ON;
}
unsafe extern "C" fn mod_evhost_parse_host(
    mut key: *mut buffer,
    mut host: *mut array,
    mut authority: *mut buffer,
) {
    let mut ptr: *mut libc::c_char = ((*authority).ptr)
        .offset(buffer_clen(authority) as isize);
    let mut colon: *mut libc::c_char = ptr;
    let mut first: libc::c_int = 1 as libc::c_int;
    let mut i: libc::c_int = 0;
    if *(*authority).ptr as libc::c_int == '[' as i32 {
        colon = ptr;
        if *ptr.offset(-(1 as libc::c_int) as isize) as libc::c_int != ']' as i32 {
            loop {
                ptr = ptr.offset(-1);
                if !(ptr > (*authority).ptr
        ...
        array_set_key_value(
            ...
            ptr,
            colon.offset_from(ptr) as libc::c_long as uint32_t,
        );
        return;
    }
    while ptr > (*authority).ptr {
        if *ptr as libc::c_int == '.' as i32 {
            if !(first != 0) {
                break;
            }
    ...
    if colon != (*authority).ptr {
        ptr = colon.offset(-(1 as libc::c_int as isize));
        i = 1 as libc::c_int;
        while ptr > (*authority).ptr {
            if *ptr as libc::c_int == '.' as i32 {
                if ptr != colon.offset(-(1 as libc::c_int as isize)) {
                    buffer_copy_string_len(
                        key,
                        b"%\0" as *const u8 as *const libc::c_char,
                    ...
                    let fresh1 = i;
                    i = i + 1;
                    buffer_append_int(key, fresh1 as intmax_t);
                    array_set_key_value(
                        host,
                        (*key).ptr,
                        buffer_clen(key),
                        ptr.offset(1 as libc::c_int as isize),
                        (colon.offset_from(ptr) as libc::c_long
        ...
        if colon != ptr {
            ...
            buffer_append_int(key, i as intmax_t);
            array_set_key_value(
                host,
                (*key).ptr,
                buffer_clen(key),
...
unsafe extern "C" fn mod_evhost_build_doc_root_path(
    mut b: *mut buffer,
    mut parsed_host: *mut array,
    mut authority: *mut buffer,
    mut path_pieces: *const buffer,
) {
    ...
    loop {
        ...
        if *ptr as libc::c_int == '%' as i32 {
            ...
            } else if *ptr.offset(1 as libc::c_int as isize) as libc::c_int == '_' as i32
                {
                let mut colon: *mut libc::c_char = strchr((*authority).ptr, ':' as i32);
                if colon.is_null() {
                    buffer_append_buffer(b, authority);
                } else {
                    buffer_append_string_len(
                        b,
                        (*authority).ptr,
                        colon.offset_from((*authority).ptr) as libc::c_long as size_t,
                    );
                }
            } else if *ptr.offset(1 as libc::c_int as isize) as libc::c_int == '{' as i32
                {
                let mut s: [libc::c_char; 3] = *::std::mem::transmute::<
                    &[u8; 3],
                    &mut [libc::c_char; 3],
                >(b"% \0");
                s[1 as libc::c_int as usize] = *ptr.offset(2 as libc::c_int as isize);
                ...
                if !ds.is_null() {
                    if *ptr.offset(3 as libc::c_int as isize) as libc::c_int
                        ...
                        || *ptr.offset(4 as libc::c_int as isize) as libc::c_int
                            == '0' as i32
                    {
                        buffer_append_buffer(b, &(*ds).value);
                    } else if (*ptr.offset(4 as libc::c_int as isize) as libc::c_int
                            - '0' as i32) as size_t
                            <= buffer_clen(&(*ds).value) as libc::c_ulong
                        {
                        buffer_append_string_len(
                            b,
                            ((*ds).value.ptr)
            ...
            buffer_append_buffer(b, path_pieces);
        }
        path_pieces = path_pieces.offset(1);
    }
    buffer_append_slash(b);
}
unsafe extern "C" fn mod_evhost_uri_handler(
    r: *mut request_st,
    mut p_d: *mut libc::c_void,
) -> handler_t {
    ...
    if ((*p).conf.path_pieces).is_null() {
        return HANDLER_GO_ON;
    }
    let b: *mut buffer = (*r).tmp_buf;
    mod_evhost_build_doc_root_path(
        b,
        &mut (*p).split_vals,
        &mut (*r).uri.authority,
        (*p).conf.path_pieces,
...
#[no_mangle]
pub unsafe extern "C" fn mod_evhost_plugin_init(mut p: *mut plugin) -> libc::c_int {
    (*p).version = 0x10440 as libc::c_int as size_t;
    (*p).name = b"evhost\0" as *const u8 as *const libc::c_char;
    (*p)
        .init = ::std::mem::transmute::<
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
        Option::<unsafe extern "C" fn() -> *mut libc::c_void>,
    >(Some(mod_evhost_init as unsafe extern "C" fn() -> *mut libc::c_void));
    (*p)
        .set_defaults = Some(
    ...
    (*p)
        .cleanup = Some(
        mod_evhost_free as unsafe extern "C" fn(*mut libc::c_void) -> (),
    );
    ...
    cpk = [
        {
            let mut init = config_plugin_keys_t {
                k: b"evhost.path-pattern\0" as *const u8 as *const libc::c_char,
